...
public:
	...
	GeneralHeader* generalHeader;
	void BlockLoader(uint64_t bytes, Type t, uint32_t version) : mem(new uint64_t[getHeaderSize() + (bytes + 7) /8]){
		...
		securityHeader = (SecurityHeader*)(mem + sizeof(GeneralHeader)/8);
	}
	enum class Type
		{
		 gismap,
...
class BlockMapLoader : public BlockLoader, public Shape {
private:
